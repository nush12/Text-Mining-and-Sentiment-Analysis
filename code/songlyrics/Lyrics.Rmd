---
title: "Lyrics"
author: "Ashish Gyanchandani and Anusha CM"
...
wordcloud(names(term_frequency), term_frequency,
          max.words = 40, colors = c("grey80", "darkgoldenrod1",
                                     "tomato"))
...
top50_artist <- sample_songs %>%
  group_by(artist) %>%
  summarise(total = length(song)) %>%
  arrange(desc(total)) %>%
  top_n(100)
  
  wordcloud(as.character(top50_artist$artist),
            as.numeric(top50_artist$total),
...
word_pairs <- unnest_tidy %>% pairwise_count(word, artist, sort = TRUE)


(word_cor <- unnest_tidy %>%
    group_by(word) %>%
    filter(n() >= 20) %>%
    pairwise_cor(word, song) %>%
    filter(!is.na(correlation)))

word_cor %>% filter(item1 == "star") %>% arrange(desc(correlation))
...
word_cor %>%
  ...
  geom_node_text(aes(label = name, size = 0.2), repel = TRUE) 
